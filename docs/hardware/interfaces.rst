.. include:: ./../macros.txt
.. include:: ./../units.txt

.. _BMS_INTERFACES_OVERVIEW:

|bms-interfaces| Overview
=========================

LTC-based |bms-interfaces|
--------------------------

The changelog for the |ltc-ltc6820|\ -based |bms-interfaces| can be found at:

.. toctree::
    :maxdepth: 1

    ./interfaces/ltc-ltc6820-vx.x.x/ltc-ltc6820-changelog

The documentation for the different versions of the |ltc-ltc6820|\ -based
|bms-interfaces| can be found at:

.. toctree::
    :maxdepth: 1

    ./interfaces/ltc-ltc6820-vx.x.x/ltc-ltc6820-v1.0.3

NXP-based |bms-interfaces|
--------------------------

The changelog for the |nxp-mc33664|\ -based |bms-interfaces| can be found at:

.. toctree::
    :maxdepth: 1

    ./interfaces/nxp-mc33664-vx.x.x/nxp-mc33664-changelog

The documentation for the different versions of the |nxp-mc33664|\ -based
|bms-interfaces| can be found at:

.. toctree::
    :maxdepth: 1

    ./interfaces/nxp-mc33664-vx.x.x/nxp-mc33664-v1.0.2

MAXIM-based |bms-interfaces|
----------------------------

The changelog for the |max-max17841b|\ -based |bms-interfaces| can be found at:

.. toctree::
    :maxdepth: 1

    ./interfaces/maxim-max17841b-vx.x.x/maxim-max17841b-changelog

The documentation for the different versions of the |max-max17841b|\ -based
|bms-interfaces| can be found at:

.. toctree::
    :maxdepth: 1

    ./interfaces/maxim-max17841b-vx.x.x/maxim-max17841b-v1.0.0
