...
For debugging the |foxbms| application with the Segger tools the
...
`https://www.segger.com <https://www.segger.com/products/development-tools/ozone-j-link-debugger/>`_.

Segger Ozone Manual
^^^^^^^^^^^^^^^^^^^

Refer to the
`Ozone manual <https://www.segger.com/downloads/jlink/UM08025>`_
for working with the Segger debugger and tools.
...
updated variable watches should mitigate this issue.
...
This can be achieved by setting a data breakpoint on this variable.
...
(It uses ``.line`` and not ``.pc``, because this member is written secondly and
therefore ``.pc`` is already written when the debugger halts.)
...
directly from waf.
This allows the user to call waf with the ``install_bin`` command in order to
