...
At first the ``os`` module initializes all tasks form :ref:`ftask_task_configuration` by calling ``OS_InitializeTasks()``.
...
Furthermore it enables the user configuration of the ``vApplicationIdleHook`` by calling ``FTSK_UserCodeIdle``.
