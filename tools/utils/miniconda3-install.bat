...
@REM set miniconda installation directory
@IF not "%USERPROFILE%"=="%USERPROFILE: =%" @(
    @SET MINICONDA3_INSTALL_DIRECTORY=%SystemDrive%\miniconda3
) ELSE @(
    @SET MINICONDA3_INSTALL_DIRECTORY=%USERPROFILE%\miniconda3
)

@SET MINICONDA3_DOWNLOAD_URL=https://repo.anaconda.com/miniconda/Miniconda3-latest-Windows-x86_64.exe
@SET MINICONDA3_INSTALLER="%USERPROFILE%\Downloads\miniconda3-installer-win64.exe"

@ECHO Downloading miniconda3
@curl.exe -fsSL %MINICONDA3_DOWNLOAD_URL% -o %MINICONDA3_INSTALLER%
@ECHO Done

@ECHO Installing miniconda3
@%MINICONDA3_INSTALLER% /S /InstallationType=JustMe /AddToPath=0 /RegisterPython=0 /D=%MINICONDA3_INSTALL_DIRECTORY%
@ECHO Done
